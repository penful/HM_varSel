lmbasic.cont.MISS  <- function(Y,k,start=0,modBasic=0,tol=10^-10,maxit=1000,out_se=FALSE,piv=NULL,Pi=NULL,Mu=NULL,Si=NULL,indb=NULL,fort=TRUE){
  ...
  if(k == 1){
    ...
    if(miss){
      ...
      for (i in 1:n) for(t in 1:TT){
        indo = R[i,t,]
        if(sum(indo)==1){
          lk = lk + ldnorm1(Y[i,t,indo],Mu[indo],Si[indo,indo])
        }else if(sum(indo)>1){
          lk = lk + ldmvnorm1(Y[i,t,indo],Mu[indo],Si[indo,indo])
        }
      }
      it = 0; lko = lk
      par = c(c(Mu),c(Si))
      ...
      while(((lk-lko)/abs(lko)>tol | it==0) & it<1000){
        ...
        par = c(c(Mu),c(Si))
        if(it%%100==0) cat(sprintf("%11g",c(mod,k,start,it,lk,lk-lko,max(abs(par-paro)))),"\n",sep=" | ")
        if(lk-lko<(-1)) browser()
      }
      if(it%%100>0) cat(sprintf("%11g",c(mod,k,start,it,lk,lk-lko,max(abs(par-paro)))),"\n",sep=" | ")
      cat("------------|-------------|-------------|-------------|-------------|-------------|-------------|\n");
    ...
    np = k*r+r*(r+1)/2
    aic = -2*lk+np*2
    bic = -2*lk+np*log(n*TT)   
    Mu = matrix(Mu,r,k)
    nameY <- dimnames(Y)[[3]]
    dimnames(Mu) <- list(nameY,state=1:k)
    out = list(lk=lk,piv=piv,Pi=Pi,Mu=Mu,Si=Si,np=np, k = k, aic=aic,bic=bic,lkv=NULL,V=NULL,n = n,
               TT = TT, modBasic = mod)
    if(miss){
      out$Y = Y
      out$Yimp = Yimp  
...
#---- Starting values ----
  ...
  if(start == 0){
    # Yimp = Yv
    # for(j in 1:r) Yimp[is.na(Yimp[,j]),j] = mean(Yimp[,j],na.rm=TRUE)
    # out = kmeans(Yimp,k,nstart = 200)
    # Mu = out$centers
    # Si = matrix(0,r,r)
    # for(u in 1:k){
    #   Tmp = Yv[out$cluster==u,]-rep(1,sum(out$cluster==u))%o%Mu[u,]
    #   Si = Si+t(Tmp)%*%Tmp/nt
    # }
  ...
  if(start==2){
    ...
    while(((lk-lko)/abs(lk)>tol & it<maxit) | it==0){
      ...
      if(miss){
        ...
        Mu00 = Mu; itc = 0 
        while((max(abs(Mu00-Mu))>tol || itc==0) & itc<10){
          Mu00 = Mu; itc = itc+1
      ...
      #  print(c(itc,max(abs(Mu-Mu00))))
        ...
        while((max(abs(Mu00-Mu))>tol || itc==0) & itc<10){
          ...
          Si = matrix(0,r,r)
          for(u in 1:k) Si= Si+ t(Yv-rep(1,n*TT)%*%t(Mu[,u]))%*%(Vv[,u]*as.matrix(Yv-rep(1,n*TT)%*%t(Mu[,u]))) 
          Si = Si/(n*TT)
          if(length(indb)==0) Mu00 = Mu
        }
      }
      ...
      lk = out$lk; Phi = out$Phi; L = out$L; pv = out$pv
      if(it/100 == round(it/100)) cat(sprintf("%11g",c(mod,k,start,it,lk,lk-lko,max(abs(par-paro)))),"\n",sep=" | ")
      lkv = c(lkv,lk)
    ...
    if(it/100 > floor(it/100)) cat(sprintf("%11g",c(mod,k,start,it,lk,lk-lko,max(abs(par-paro)))),"\n",sep=" | ")
    # adjust output
    #	if(any(yv!=1)) V = V/yv
