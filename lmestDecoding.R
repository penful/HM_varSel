lmestDecoding <- function(object,...) {
  UseMethod("lmestDecoding")
}


lmestDecoding.LMbasic <- function(est, sequence = NULL, formula = NULL,
                                   newdata = NULL, index = NULL)
{
  ...
  if(is.null(formula))
  {
    ...
    temp <- LMest:::getResponses(data = data.new,formula = formula)
    Y <- temp$Y
    Xmanifest <- temp$X
    Xinitial <- NULL
    Xtrans <- NULL
  ...
  tmp <- LMest:::long2matrices.internal(Y = Y, id = id, time = tv, yv = rep(1,max(id)),
  ...
  k = length(est$piv)
  out = LMest:::complk(Y,R,rep(1,n),piv,Pi,Psi,k)
...
lmestDecoding.LMmixed <- function(est, sequence = NULL, formula = NULL,
                                   newdata = NULL, index = NULL, fort = TRUE)
...
lmestDecoding.LMmanifest <- function(est, sequence = NULL, formula = NULL,
                                      newdata = NULL, index = NULL)
{
  ...
  if(is.null(formula))
  {
    S <- data.new
    formula <- attributes(est)$responsesFormula
    temp <- LMest:::getResponses(data = est$data,formula = formula)
    X <- temp$X
    Xinitial <- NULL
    Xtrans <- NULL
  }else{
    ...
    {
      temp <- LMest:::getResponses(data = est$data,formula = attributes(est)$responsesFormula)
  ...
  tmp <- LMest:::long2matrices.internal(Y = S, id = id, time = tv, yv = rep(1,max(id)),
  ...
  if(k>1){
    for(c in 1:k){
      ...
      if(is.null(G2)) G2 = G2c else if(k==2) G2 = LMest:::blkdiag(matrix(G2,ncol=1),matrix(G2c,ncol=1)) else G2 = LMest:::blkdiag(G2,G2c)
      if(is.null(H2)) H2 = H2c else if(k==2) H2 = LMest:::blkdiag(matrix(H2,nrow=1),matrix(H2c,nrow=1))else H2 = LMest:::blkdiag(H2,H2c)
  ...
  while(cont && itg<5){
    ...
    for(c in 1:k){
      ...
      for(d in 1:q){
        ...
        Qv1 = LMest:::expit(Eta1); Qv1 = pmin(pmax(Qv1,10^-100),1-10^-100)
    ...
    lk = sum(log(pim))
    # E-step
    out = LMest:::rec3(Q,yv=rep(1,n),PIs,Pio,pim)
    U = out$U; V = out$V
    # M-step: latent parameters
    if(k>1){
      u1 = Mar%*%rowSums(U[,,1])
      V1 = Mar%*%V%*%t(Mar)
      out = LMest:::lk_sta(tau,as.vector(u1),V1,G2,outl=TRUE)
  ...
  out = LMest:::lk_obs_manifest(par1,S,Xd,yv=rep(1,n),indn,lev,k,sup,G2,IPI,mod=0,outp=TRUE)
  lk = out$lk; U = out$U
  sup1 = t(Mar)%*%al
...
lmestDecoding.LMlatent <- function(est, sequence = NULL, formula = NULL,
                                      newdata = NULL, index = NULL, fort = TRUE)
{
  ...
  if(is.null(formula))
  {
    temp <- LMest:::getResponses(data = data.new,formula = attributes(est)$responsesFormula)
    Y <-  temp$Y
       temp <- LMest:::getLatent(data = data.new,responses = attributes(est)$responsesFormula,
    ...
    if(is.null(formula[[3]]))
    {
      temp <- LMest:::getResponses(data = data.new,formula = formula)
      X <- temp$X
      temp <- LMest:::getLatent(data = data.new,latent = attributes(est)$latentFormula,
      ...
      temp <- LMest:::getResponses(data = data.new,formula = formula)
      Y <- temp$Y
      temp <- LMest:::getLatent(data = data.new,latent = formula,
                                responses = formula)
      Xinitial <- temp$Xinitial
      Xtrans <- temp$Xtrans
    }
  ...
  be = as.vector(est$Be)
  out = LMest:::prob_multilogit(XXdis,be,Xlab,fort)
  Piv = out$P
  ...
  if(param=="multilogit"){
    ...
    for(h in 1:k){
      out = LMest:::prob_multilogit(ZZdis[,,,h],Ga[,h],Zlab,fort)
    ...
    out = LMest:::prob_multilogit(ZZdis,Ga,Zlab,fort)
    PIdis = out$Pdis;
    Tmp = array(out$P,c(k,n,TT-1,k))
    PI[,,,2:TT] = aperm(Tmp,c(1,4,2,3))
  }
  out = LMest:::lk_comp_latent(Y,R,rep(1,n),Piv,PI,Psi,k,fort=fort)
  Phi = out$Phi; L = out$L; pv = out$pv

  out = LMest:::prob_post_cov(Y,rep(1,n),Psi,Piv,PI,Phi,L,pv,fort=fort)
  V = out$V
...
lmestDecoding.LMbasiccont <- function(est, sequence = NULL, formula = NULL,
                                  newdata = NULL, index = NULL)
{
  ...
  k = length(est$piv)
  out = LMest:::complk_cont_miss(Y = Y,R,piv,Pi,Mu,Si,k)
  Phi = out$Phi; L = out$L; pv = out$pv
  V = array(0,c(n,k,TT))
  M = matrix(1,n,k)
